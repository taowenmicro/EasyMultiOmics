#' @title lasso model screening of characteristic genes
#' @description
...
#' res =lasso.metf (ps =ps, top = 100, seed = 1010, k = 5)
#' accuracy = res[[1]]
#' accuracy
#' importance = res[[2]]
#' importance
lasso.metf <- function(ps= ps, top = 20, seed = 1010, k = 5) {
  set.seed(seed)
  # 数据准备
  ...
  for (i in 1:k) {
    ...
    if (length(levels(test$OTUgroup))>2) {
      ...
      names(importance) <- rownames(coef(lasso_model,s = "lambda.min")[[1]])[-1]}else{
        importance <- as.vector(coef(lasso_model, s = "lambda.min")[-1])
