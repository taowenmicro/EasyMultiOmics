...
ps01_2 =  ps01 %>%  phyloseq::subset_samples(Group%in% Desep_group ) %>%
                 phyloseq::subset_taxa(row.names( phyloseq::tax_table(ps01)) %in% dat$id)

ps02_2 = ps02 %>%  phyloseq::subset_samples(Group%in% Desep_group ) %>%
  phyloseq::subset_taxa(row.names( phyloseq::tax_table(ps02)) %in% dat$id)
