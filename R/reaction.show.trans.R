...
#' res3 = reaction.show.trans(ps = ps.trans3, dif.method = "wilcox")
#' res3$Plots$OE.WT.plot
#' dat = res3$Plotdata$OE.WT
#' head(dat)
#' }

reaction.show.trans = function(
    ps,
    dif.method = "wilcox"
){
  ...
  for (i in 1:dim(com)[2]) {
    ...
    x <- clusterProfiler::enricher(gene = c(paste0("ko:",id.tem)),
