...
Ven.Upset.metm <- function(otu = NULL, tax = NULL, map = NULL, tree = NULL,
                           ps = NULL, group = "Group", N = 0.5, size = 3,
                           fill_color = NULL) {
  ...
  # 使用自定义颜色，如果未提供则使用默认颜色
  if (is.null(fill_color)) {
    palette_nature <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728",
                        "#9467bd", "#8c564b", "#17becf")
    fill_color <- palette_nature[seq_along(ven3)]
  } else {
    # 按照 ven3 的名称顺序排列颜色
    fill_color <- fill_color[names(ven3)]
  }

  p <- ggvenn::ggvenn(ven3, fill_color = fill_color,
                      fill_alpha = 0.6, stroke_size = 0.8, set_name_size = 5) +
    ggplot2::theme_bw()
