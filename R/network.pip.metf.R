#' @title Metagenome functional composition related network
#' @description This function is designed to generate graphical representations related to network analysis and compute various network properties.
...
#' @param N filter metagenome functional composition tables by abundance.The defult, N=0, extract the top N number relative abundance of gene. e.g 100
...
#' @param big TRUE or FALSE the number of micro data was so many (> 300),you can chose TRUE
#' @examples
#' library(ggClusterNet)
#' library(igraph)
#' data(ps.kegg)
#' result = network.pip.metf(N = 400,big = TRUE,select_layout = FALSE,layout_net = "model_maptree2",
#' r.threshold = 0.6,p.threshold = 0.05,maxnode = 2,label = FALSE,lab = "elements",group = "Group",
#' size = "igraph.degree",zipi = TRUE,ram.net = TRUE,clu_method = "cluster_fast_greedy",step = 100,
#' R=10,ncpus = 1,fill = "Pathway")
#' dat = result[[2]]
#' cortab = dat$net.cor.matrix$cortab
#' plot = result[[1]]
#' p0 = plot[[1]]
#' p0
#' @return list which contains gene correlation matrix and ggplot2 plot.
#' @author Contact: Tao Wen \email{taowen@@njau.edu.cn};Penghao Xie \email{2019103106@@njau.edu.cn}.
#' @export
network.pip.metf = function(
    otu = NULL,
    tax = NULL,
    map = NULL,
    ps = NULL,
