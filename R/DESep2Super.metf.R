...
#'
#' @description
#' The `DESep2Super.metf` function performs differential abundance analysis between metagenome functional groups using the `DESeq2` package.
#' It identifies significantly enriched or depleted taxa across multiple group comparisons by calculating log2 fold changes,
#' raw p-values, and adjusted p-values (FDR). The input data can be provided as a `phyloseq` object or as separate metagenome functional composition, taxonomic, and metadata tables.
#'
...
#' @param tax  Metagenome functional classification table.Optional if `ps` is provided.
