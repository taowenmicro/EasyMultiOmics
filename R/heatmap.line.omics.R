...
heatmap.line.omics = function(ps01 = ps,
                              psG = ps.ms
  ...
  # ggsave("cs17.pdf",p1,width = 6,height = 6)
  ...
  p3 = ggplot(pcm0, aes(y = id, x = variable)) +
    ...
    ggh4x::facetted_pos_scales(
      y = list(
        group == "Metabolites" ~ scale_y_discrete(position = "right")
      )
    )
  ...
  head(edge)
  dim(edge)
  edge$from = gsub("^16S_","",edge$OTU_2,perl = TRUE)
  edge$to = gsub("^Metabolites_","",edge$OTU_1,perl = TRUE)
  head(node4)
  node5 = node4 %>%
    dplyr::filter(variable == "KO40_1",group == "16S")
  head(node5)
  ...
  node6 = node4 %>%
    dplyr::filter(variable == "WT5_2",group == "Metabolites")
  head(node6)
