...
stemp_diff.metm <- function(ps = ps, Top = 20, ranks = 6, method = "TMM",
                            test.method = "t.test", col.g = NULL) {
  ...
  data <- data %>% dplyr::inner_join(as.tibble(phyloseq::sample_data(ps)), by = "ID")
  ...
  if (is.null(col.g)) {
    library(ggsci)
    col.g <- pal_npg()(length(group_levels))
    names(col.g) <- group_levels
  }

  cbbPalette <- as.character(col.g[group_levels])
  ...
  diff.mean$Group <- c(ifelse(diff.mean$estimate > 0, group_levels[1], group_levels[2]))
  ...
  p3 <- ggplot(diff.mean, aes(var, estimate, fill = Group)) +
    ...
    geom_text(aes(x = nrow(diff.mean)/2 + 0.5, y = 0.85),
              label = "P-value (corrected)", srt = 90, fontface = "bold", size = 5) +
    coord_flip() +
    ylim(c(0, 1)) +
    theme(
