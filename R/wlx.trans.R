#' @title Differential abundance analysis using wilcox test for Transcriptome functional data
#' @description
#' #' This function performs differential abundance analysis for Transcriptome functional data using the `wilcox.test` function.
...
#' res = wlx.trans(ps = ps,group  = "Group",artGroup =NULL,group  = "Group",
...
wlx.trans = function(
  ...
  for (i in 1:dim(aaa)[2]) {
    ...
    result=data.frame(ID=colnames(data)[2:ncol(data)],
                      p=sapply(data[,2:ncol(data)],function(x){wilcox.test(x~data$Group)[["p.value"]]}),
                      logFC=log2FC)
    #padjust
