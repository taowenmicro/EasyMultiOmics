#' @title Decisiontree model screening of characteristic microorganisms
#' @description
...
#' library(caret)
#' library(rpart)
#' res = decisiontree.micro(ps=ps, top = 20, seed = 1010, k = 5)
#' accuracy = res[[1]]
#' accuracy
#' importance = res[[2]]
#' importance

decisiontree.micro <- function(ps=ps, top = 20, seed = 6358, k = 5) {
  set.seed(seed)
