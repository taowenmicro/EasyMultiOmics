...
t.trans= function(
  ...
  for (j in 1:dim(aaa)[2]) {
    ...
    p <- ggplot(x,aes(x =logFC ,y = -log2(p), colour=level)) +
      ...
      geom_vline(xintercept=c(-1,1),
                 ...
                 color = 'black',
                 size = 0.5) +
      ggrepel::geom_text_repel(data=x, aes(x=logFC, y=-log2(p), label=row.names(x)), size=1) +
  ...
  norm2= t(iris.combine) %>%
    as.data.frame()
  x = cbind(x,norm2)
  # data_list[["combined_table"]] <- x
  return(list(plot_list,x))
}



T <- function(a = a,rep = rep){
  ...
  for(i in 1:nrow(a)){
    ...
    if(sd(a[i,(1:rep)])==0&&sd(a[i,(rep+1):(rep*2)])==0){
      ...
      y = stats::t.test(as.numeric(a[i,(1:rep)]),as.numeric(a[i,(rep+1):(rep*2)]))
      Pvalue[i]<-y$p.value
