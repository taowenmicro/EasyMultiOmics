#' @title For cross-validation of metabolite data
#' @description For cross-validation of metabolite data.
#' @param otu metabolite composition table.
#' @param map Sample metadata.
#' @param tax taxonomy table.
#' @param ps phyloseq object of metabolite.
#' @param Group column name for groupID in map table.
#' @param optimal important metabolite number which selected.
...
#' @examples
#' library(dplyr)
#' ps_rela <- ps.ms%>%phyloseq::transform_sample_counts(function(x) {x/sum(x)} )
#' result = rfcv.ms(otu = NULL,tax = NULL,map = NULL,tree = NULL ,ps = ps_rela,group  = "Group",optimal = 20,nrfcvnum = 6)
#' prfcv = result[[1]]# plot rfcv
# result[[2]]# plotdata
#' rfcvtable = result[[3]]# table rfcv
#'@export
rfcv.ms = function(otu = NULL,tax = NULL,map = NULL,tree = NULL,
