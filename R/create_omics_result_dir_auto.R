...
# -------------------------------------------------------------------
# Public: auto-detect (amplicon vs metabolome) and create result dir
# -------------------------------------------------------------------

#' Automatically create result directory for amplicon or metabolome data
#'
#' This function uses \code{guess_phyloseq_data_type()} to determine
#' whether a \code{phyloseq} object is amplicon-style or metabolomics-style,
#' and then dispatches to the appropriate directory-creation function:
#'
#' \itemize{
#'   \item \code{create_metabolome_result_dir()} – when type is \code{"metabolome"};
#'   \item \code{create_amplicon_result_dir()}  – when type is \code{"amplicon"}.
#' }
#'
#' Note: \code{create_amplicon_result_dir()} is assumed to be defined
#' elsewhere in your codebase (e.g. another utils file).
#'
#' @param ps A \code{phyloseq} object.
#' @param base_dir Base directory in which to create the result folder.
#' @param include_time Logical; if \code{TRUE}, append \code{HHMMSS} to
...
#' @examples
#' \dontrun{
#'   out_dir <- create_omics_result_dir_auto(ps, base_dir = "./result")
#' }
#'
#' @export
create_omics_result_dir_auto <- function(ps,
  ...
  if (identical(data_type, "metabolome")) {
    return(create_metabolome_result_dir(
      ps,
      base_dir     = base_dir,
      prefix       = "metabolome",
  ...
  if (identical(data_type, "amplicon")) {
    if (!exists("create_amplicon_result_dir")) {
      stop("Data type is 'amplicon' but `create_amplicon_result_dir()` ",
           "is not available in the current environment.")
    }
    # Call your existing amplicon directory creator
    return(create_amplicon_result_dir(
      ps,
      base_dir     = base_dir,
      prefix       = "amplicon",
      include_time = include_time
  ...
  stop("Cannot classify this phyloseq object as 'amplicon' or 'metabolome': ",
       "guess_phyloseq_data_type(ps) == 'unknown'.")
}
