...
ps.micro  = readRDS("../20250922集中分析一批次多组学项目测试asyMultiOmics包/刘嘉伟/data/ps_bac.rds")
ps.micro = ps.micro %>% change.rank.name()
map= sample_data(ps.micro)
head(map)
# 如果要存回数据框
map$Group2 = map$Group
map$Group <- map$Treatment
...
# 保存路径设置---
path =  "../20250922集中分析一批次多组学项目测试asyMultiOmics包/刘嘉伟//result.16S/"
fs::dir_create(path)
...
result = MuiaovMcomper2(data = data,num = 3:6)

result1 = EasyMultiOmics::FacetMuiPlotresultBox(data = data,num = 3:6,
                                                result = result,
...
res = EasyMultiOmics::FacetMuiPlotresultBar(data = data,num = c(3:6),
...
res = EasyMultiOmics::FacetMuiPlotReBoxBar(data = data,num = c(3:6),result = result,sig_show ="abc",ncol = 4,
                                           mult.y = 0.3,
                                           lab.yloc = 1.1
)
p1_3 = res[[1]]+
...
result = ordinate.metm(ps = ps.micro, group = c("Group","Group2"), dist = "bray",
...
#  机器学习网络构建#-----------------
res = rf_network_from_phyloseq(ps = ps.micro %>% filter_OTU_ps(50),
                               ntree = 500,
...
head(edges)
# 1. 过滤显著边
edges_filt <- edges %>% filter(pval < 0.05)

# 2. 生成 igraph 对象
g <- graph_from_data_frame(edges_filt[, c("target", "predictor", "importance")],
                           directed = FALSE)

# 3. 转换为邻接矩阵
cor_mat <- as_adjacency_matrix(g, attr = "importance", sparse = FALSE)

library(ggClusterNet)

# 用 model_maptree2 进行模块划分（常用：cluster_fast_greedy / walktrap）
mod_res <- model_maptree2(cor = cor_mat,
                          method = "cluster_fast_greedy")
node <- mod_res[[1]]  # 包含节点和模块分组
head(node)


edge = edgeBuild(cor = cor_mat,node = node)
head(node)
pnet <- ggplot() + geom_segment(aes(x = X1, y = Y1, xend = X2, yend = Y2,color = as.factor(cor)),
                                data = edge, size = 1,alpha = 0.1) +
  geom_point(aes(X1, X2),pch = 21, data = node) +
  ggrepel::geom_text_repel(aes(X1, X2,label = elements ), data = node) +
...
res <- ML.network(
  ...
  scale.method = "l1",
  method = c("rf","xgb","glmnet","svm","lgbm","mlp","gam")[6],
  ntree = 100,
  p.threshold = 0.05,
  n_perm = 10,
  n_cores = 10
