...
result1 = EasyStat::FacetMuiPlotresultBox(data = data,num = 3:5,
                                          result = result,
                                          sig_show ="abc",ncol = 4 )
p1_1 = result1[[1]]+theme_bw()+
  # scale_x_discrete(limits = axis_order) +
  # mytheme1 +
  guides(fill = guide_legend(title = NULL)) +
  scale_fill_manual(values = colset4)+ylab("Gene diversity")
p1_1

res = EasyStat::FacetMuiPlotresultBar(data = data,num = c(3:(5)),
                                      result = result,
                                      sig_show ="abc",ncol = 4)
p1_2 = res[[1]]+theme_bw()+
  # scale_x_discrete(limits = axis_order) +
  # mytheme1 +
  guides(fill = guide_legend(title = NULL)) +
  scale_fill_manual(values = colset1)+ylab("Gene diversity")
p1_2
...
p1_0 = result1[[2]] %>% ggplot(aes(x=group , y=dd )) +
  ...
  # theme_classic()+
  geom_text(aes(x=group , y=y ,label=stat))+theme_bw()+
  # scale_x_discrete(limits = axis_order) +
  # mytheme1 +
  guides(fill = guide_legend(title = NULL)) +
  scale_fill_manual(values = colset1)+ylab("Gene diversity")
...
rare <- mean(sample_sums(ps.metf))/10
result = alpha.rare.line.metf(ps = ps.metf, group = "Group", method = "Richness", start = 100, step = rare)
#--提供单个样本稀释曲线的绘制
p2_1 <- result[[1]]
#--提供单个样本稀释曲线的绘制
p2_1 <- result[[1]] +
...
#---每个部分
result = VenSuper.metf(ps = ps.metf,
                       group = "Group",
                       num =6 )
...
library(e1071)
result =rfcv.metf(ps = ps.metf %>% filter_OTU_ps(200),group  = "Group",optimal = 20,nrfcvnum = 6)
prfcv = result[[1]]
prfcv
# result[[2]]# plotdata
rfcvtable = result[[3]]
...
res = randomforest.metf( ps = ps.metf,group  = "Group", optimal = 50 ,fill ="Pathway"  )
p42.1 = res[[1]]
p42.1
p42.2 =res[[2]]
p42.2
...
num =10
phyloseq::tax_table(ps.cnps)
result = VenSuper.metf(ps = ps.cnps,
                       group = "Group",
                       num =10,
                       j= "gene" )
...
#23 glm.metf:glm筛选特征功能----
res <- glm.metf(ps = ps %>% filter_OTU_ps(50), k = 5)
AUC = res[[1]]
AUC
importance = res[[2]]
importance
