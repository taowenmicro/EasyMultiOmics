...
sample_data(ps03)
pst = ps03 %>% subset_samples(Group %in% group)
...
#constraint analysis----
ps.tem = ps16s %>% tax_glom_wt("Genus")
# 真菌筛选
res = loadingPCA.micro(ps = psITS%>% tax_glom_wt("Genus"))
# p = res[[1]]
# p
dat = res[[2]]
...
ftab = psITS%>% tax_glom_wt("Genus") %>% scale_micro() %>%
  phyloseq::subset_taxa(
    row.names(phyloseq::tax_table(psITS%>% tax_glom_wt("Genus"))) %in% c(id)
  ) %>% vegan_otu() %>%
  as.data.frame()
...
#9 rdacca.hp.micro:   rdacca.hp.micro.p  层次分割对细菌群落影响的真菌---------
library(rdacca.hp)
library(vegan)
...
res =xgboost.omics(ps = ps.cs, top = 5,k =5  )
accuracy = res[[1]]
accuracy
importance = res[[2]]
importance
...
#14 kNN: 筛选特征细菌与真菌--------
res = knn.omics(ps =pst, top = 20,k =5 )
accuracy = res[[1]]
accuracy
importance = res[[2]]
importance
...
res =bagging.omics(ps =  pst, top = 20, seed = 1010, k = 5)
accuracy = res[[1]]
accuracy
importance = res[[2]]
importance
...
#17 lasso: 筛选特征细菌与真菌-----
res = lasso.omics (ps =  pst, top = 20, seed = 10, k = 5)
accuracy = res[[1]]
accuracy
importance = res[[2]]
importance
